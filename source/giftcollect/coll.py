...
class DB:
    ...
    VALUES ((?), (?), (?), (?), (?), (?))'''
    DELETE_ITEM = '''DELETE FROM [gifts] WHERE [date] = (?)'''
...
class Collector:
    ...
    def count_url(self, _pid):
        ...
        self.db.delete(DB.DELETE_ITEM, latest_timestamp)
        if latest_timestamp:
        ...
        with open('{}/清单_{}.csv'.format(OUTPUT, _pid), "w+", encoding='utf-8') as fn:
            ...
            for data in data_list:
                p = PRICE.get(data[3])
                if p is not None:
                    sp = float(data[4]) * float(p)
                    p = float(p)
                else:
                    print('未收录的礼物: {}'.format(data[3]))
                    p = 0
                if data[2] not in RATIO['特定用户']:
                    r = RATIO['非特定用户比例'][0]
                elif p >= float(RATIO['指定价格'][0]):
                    r = RATIO['高比例'][0]
                else:
                    r = RATIO['低比例'][0]
                r = float(r)
                text = "{},{},{},{},{},{},{},{}\n".format(*data, sp, r, sp * r)
                fn.write(text)
