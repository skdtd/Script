...
function findmulticolor(c)
	...
	return findMultiColorInRegionFuzzy(c[1],c[2],c[3],c[4],c[5],c[6],c[7]);
end
...
function FreeGame:page(colors)
	if not colors or  type(colors) ~= type({}) then
		error('颜色库不存在自己找为啥')
	end
	self.page_p = colors;
...
function FreeGame:getPoints(ts)
	...
	elseif type(ts[1]) =="string"  then
		if string.find(ts[1],".png") or  string.find(ts[1],".jpg") or  string.find(ts[1],".jpeg") then
			lineprint("图片识别");
			x, y = findImageInRegionFuzzy(ts[1], ts[2], ts[3],ts[4], ts[5], ts[6], ts[7]);
...
function FreeGame:run(job,times)
	if not job  then
		error('你传进来的table 不存在请仔细检查！！！！！！不要再问为啥报错')
	end
	local runTimes =1;
	while true do
		FreeGame:cardLogic();
