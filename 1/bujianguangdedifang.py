...
def policy(mini_map):
    ...
    while True:
        ...
        if x == -1 and current_direction == RIGHT:  # 在最右侧迷失时调头
            ...
            current_direction = RIGHT
        if current_direction == LEFT and 0 < x < 55 + SCREEN_X:
            current_direction = RIGHT
        elif current_direction == RIGHT and x > 110 + SCREEN_X:
            current_direction = LEFT
        hit_and_run(current_direction, 6)


def test():
    x, y = find_mini_map(MINI_MAP["不见光的地方"], "C:/Users/Administrator/Desktop/player.bmp")
...
if __name__ == "__main__":
    ...
    keyboard.wait("F6")
    while True:
        xw, yw = policy(MINI_MAP["不见光的地方"])
        check_wheel(xw, yw, MINI_MAP["不见光的地方"])
    #
