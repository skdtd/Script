...
SERVER_HOST = None
ID_NUM = None
with open("C:/Users/Administrator/Desktop/config", "r") as f:
...
def click_screen():
    time.sleep(10)
    for _ in range(10):
        DM.MoveTo(SCREEN_X, SCREEN_Y)
        for h in range(6):
            for w in range(9):
                DM.MoveR(128, 0)
                time.sleep(0.05)
            DM.MoveTo(SCREEN_X, SCREEN_Y + ((h + 1) * 120))
        x, y = find_mini_map([SCREEN_X, SCREEN_Y, SCREEN_X1, SCREEN_Y1], "C:/Users/Administrator/Desktop/blue.bmp")
        if x != -1:
            send_alarm()
...
def policy(mini_map):
    ...
    while True:
        ...
        if x == -1 and current_direction == RIGHT:  # 在最右侧迷失时调头
            ...
            current_direction = RIGHT
        if current_direction == LEFT and 0 < x < 50 + SCREEN_X:
            current_direction = RIGHT
        elif current_direction == RIGHT and x > 125 + SCREEN_X:
            current_direction = LEFT
        if y > 77 + SCREEN_Y:
            if (current_direction == RIGHT and x < 192 + SCREEN_X) \
                    or (233 + SCREEN_X - 5 < x < 233 + SCREEN_X + 5) \
                    or (current_direction == LEFT and x > 265):
                print(x, y)
                double_jump()
...
if __name__ == "__main__":
    keyboard.wait("F6")
    Process(target=click_screen).start()
    while True:
