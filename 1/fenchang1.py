...
def find_mini_map(mini_map, target):
    ...
    b, x, y = DM.FindPic(x1 + SCREEN_X, y1 + SCREEN_Y, x2 + SCREEN_X, y2 + SCREEN_Y, target, "303030", 0.7, 0)
    return x, y
...
def check_wheel(x, y):
    ...
    DM.KeyUp(LEFT)
    while True:
        xp, yp = find_mini_map(MINI_MAP["机械坟场山丘1"], "C:/Users/Administrator/Desktop/player.bmp")
        print(xp, yp, x, y)
        if xp - x > 1:
            DM.KeyPress(LEFT)
        elif xp - x < -1:
...
def policy():
    ...
    while True:
        print("loop start")
        x, y = find_mini_map(MINI_MAP["机械坟场山丘1"], "C:/Users/Administrator/Desktop/player.bmp")
        wheel_x, wheel_y = find_mini_map(MINI_MAP["机械坟场山丘1"], "C:/Users/Administrator/Desktop/wheel.bmp")
        if wheel_x != -1:
        ...
        if x == -1 and current_direction == RIGHT:  # 在最右侧迷失时调头
            ...
            current_direction = RIGHT
        if current_direction == LEFT and 0 < x < 25:
            current_direction = RIGHT
        elif current_direction == RIGHT and x > 245:
            current_direction = LEFT
        if current_direction == RIGHT and y < 100:
            go_down()
        elif current_direction == LEFT and y > 96:
            double_jump()

        hit_and_run(current_direction)


def test():
    x, y = find_mini_map([5, 60, 185, 90], "C:/Users/Administrator/Desktop/player.bmp")
    pic_path = "C:/Users/Administrator/Desktop/test.jpg"
    x1, y1, x2, y2 = MINI_MAP["机械坟场空地"]
    DM.Capture(x1, y1, x2, y2, pic_path)
    print x - SCREEN_X, None, SCREEN_X


if __name__ == "__main__":
    keyboard.add_hotkey("F6", test)
    keyboard.wait()
# keyboard.wait("f6")
#
# while True:
